...
export default function LessonRow({ id, title, number, completed, current = false, durationSeconds, videoId, compact = false }: Props) {
    ...
    return (
        <Link aria-current={current ? 'step' : undefined} className={`group flex items-center gap-3 border border-transparent px-3 py-3 transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-500 ${current ? 'border-[#9347dd]/45 bg-[#6429aa]/25 shadow-[inset_3px_0_0_#9347dd]' : 'hover:bg-white/[0.045]'}`} href={`/lessons/${id}`}>
            {!compact && <div className="hidden h-10 w-16 shrink-0 overflow-hidden rounded-md bg-white/5 sm:block"><CourseCover title={title} videoId={videoId} /></div>}
