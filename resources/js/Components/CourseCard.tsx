...
export default function CourseCard({ course, detail }: Props) {
    return (
        <article className="overflow-hidden rounded-2xl border border-asex-border bg-white shadow-sm transition hover:-translate-y-1 hover:shadow-lg">
            <div className="flex aspect-[16/8] items-center justify-center overflow-hidden bg-ink p-6 text-center text-xl font-bold text-brand-300">
                {course.thumbnailPath ? <img className="h-full w-full object-cover" src={`/storage/${course.thumbnailPath}`} alt="" /> : <BrandLogo className="h-20 w-40" />}
            </div>
