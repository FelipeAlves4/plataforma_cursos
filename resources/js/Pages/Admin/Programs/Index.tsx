...
export default function Index({ programs }: { programs: Program[] }) {
    return <AdminLayout>
        ...
        <section className="admin-panel mt-8 overflow-hidden">
            {programs.length ? <div className="overflow-x-auto"><table className="admin-table min-w-[900px]"><thead><tr><th>Nome</th><th>Público</th><th>Cursos</th><th>Preço padrão</th><th>Status</th><th className="text-right">Ações</th></tr></thead><tbody>{programs.map((program) => <tr key={program.id}><td><strong className="text-[#F8F7FB]">{program.name}</strong><small className="mt-1 block text-[#9D93B8]">{program.offerCount} {program.offerCount === 1 ? 'oferta' : 'ofertas'} · {program.checkoutLinkCount} {program.checkoutLinkCount === 1 ? 'link' : 'links'}</small></td><td>{program.audience ?? '—'}</td><td>{program.courseCount}</td><td>{formatCurrency(program.defaultPriceCents)}</td><td><span className={`rounded-full px-3 py-1 text-xs font-bold ${program.active ? 'bg-emerald-400/15 text-emerald-200' : 'bg-white/10 text-white/55'}`}>{program.active ? 'Ativo' : 'Inativo'}</span></td><td><div className="flex justify-end gap-2"><Link className="admin-edit-button" href={`/admin/programs/${program.id}/edit`}>Editar</Link>{program.checkoutLinkCount > 0 && <Link className="admin-secondary-button !px-3 !py-2" href={`/admin/checkout-links?program_id=${program.id}`}>Ver links ({program.checkoutLinkCount})</Link>}{program.active && <><Link className="admin-secondary-button !px-3 !py-2" href={`/admin/offers/create?program_id=${program.id}`}>Disponibilizar para aluno</Link><Link className="admin-secondary-button !px-3 !py-2" href={`/admin/checkout-links?program_id=${program.id}&create=1`}>Criar link de venda</Link></>}</div></td></tr>)}</tbody></table></div> : <div className="admin-empty-state py-16"><p>Nenhum programa criado.</p><Link className="admin-secondary-link mt-4 inline-flex" href="/admin/programs/create">Criar o primeiro programa</Link></div>}
        </section>
    </AdminLayout>;
}
